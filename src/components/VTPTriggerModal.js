...
import { Button, Card, Input, InputNumber, Select, Space, Switch, Tooltip, Row, Col, Checkbox, Radio, Modal } from 'antd';
const { Option } = Select;

export default function VTPTriggerModal({ isOpenVtpTriggerModal, closeVtpTriggerModal }) {
  ...
  return (
    <Modal
      ...
      onCancel={closeVtpTriggerModal}
      footer={[
        <Button key='submit' onClick={() => saveTrigger(selectedVtpTrigger)}>저장</Button>,
        <Button key='back' onClick={closeVtpTriggerModal}>취소</Button>
      ]}
    >
      <Card style={{ postion: 'absolute', top: 0, left: 0 }}>
