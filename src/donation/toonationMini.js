...
export class ToonationMini {
  constructor(key) {
    this.key = key;

    // toonation alert setting info
    this.alertBoxUrl = "https://toon.at/widget/mini_donation/" + key;
    this.payload = '';
  ...
  async loadPayload() {
    try {
      ...
      if (200 === res.statusCode) {
        let matchedPayload = res.body.match(/window.aspx_payload = function \(\) { return ['|"]([a-zA-Z0-9]+)['|"]/);
      ...
      connection.on('message', (message) => {
        try {
          if (message.type === 'utf8') {
            ...
            if (data.content && data.code && (111 === data.code || 112 === data.code)) {  // 미니후원(code: 111 | 112)
